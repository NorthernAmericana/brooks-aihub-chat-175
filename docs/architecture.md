...
- **App Router framework**: The application is structured around the App Router, with route handlers and server components coordinating UI, API endpoints, and server-side logic in one place.
- **AI SDK**: Model calls are made through the AI SDK layer, which abstracts the provider (via a managed AI Gateway for hosted deployments) and streams tokens to the UI.
- **Data stores**:
...
1. **Client UI** sends a chat prompt to an App Router route handler.
...
- `BLOB_READ_WRITE_TOKEN` — Blob storage read/write token for file persistence.
- `AUTH_SECRET` — Secret used by the auth layer for signing or verifying sessions.
- `AI_GATEWAY_API_KEY` — API key for the AI Gateway in self-hosted or alternative deployments.
