...
async function runTests() {
  ...
  console.assert(cities.length >= 206, `Should have at least 206 entries, got ${cities.length}`);
  console.log(`✓ Test 1 passed - Loaded ${cities.length} entries (Season 1 max: 200 cities)`);

  // Test 2: Verify Season 1 cities are present
  console.log("\nTest 2: Verify Season 1 cities are present");
  const season1Cities = ["Pensacola, Florida", "Jacksonville, Florida", "Miami, Florida", "Tampa, Florida", "Orlando, Florida", "St. Petersburg, Florida", "Pace/Milton/Avalon, Florida"];
  ...
  console.log("✓ Test 2 passed - All 7 Season 1 cities present (7/200 capacity)");
