    ...
    <div className="mb-6 w-full rounded-3xl border border-emerald-500/30 bg-gradient-to-br from-emerald-500/10 via-background to-background p-4 text-left shadow-sm sm:p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {data?.installed
              ? "Spotify app installed in Brooks AI HUB."
              : "Install Spotify Music Player to enable quick controls."}
          </p>
        </div>
      ...
      {data?.installed ? (
        data.connected ? (
          ...
          <div className="mt-3 rounded-2xl border border-amber-300/30 bg-amber-500/15 p-3 text-xs text-amber-100">
            Spotify installed, but account not connected. Open the app to finish
            OAuth.
            <div>
              <Link
                className="mt-2 inline-flex rounded-full border border-amber-300/40 px-3 py-1"
                href="/Spotify"
              >
                Connect Spotify
              </Link>
