...
const apps = [
  {
    ...
    installKey: INSTALL_KEYS.brooksBears,
  ...
  },
  {
    ...
    installKey: INSTALL_KEYS.myCarMindAto,
...
export default function StorePage() {
  ...
  const [installedApps, setInstalledApps] = useState<InstallState>({});
  ...
  useEffect(() => {
    ...
    const refreshInstallState = () => {
      ...
      for (const app of apps) {
        if (app.installKey) {
          nextState[app.installKey] =
            window.localStorage.getItem(app.installKey) === "true";
        }
      }
      setInstalledApps(nextState);
    };
      ...
      <div className="store-content flex-1 overflow-y-auto -webkit-overflow-scrolling-touch touch-pan-y overscroll-behavior-contain px-4 pb-6">
        <div className="space-y-4">
          {filteredApps.map((app) => {
            const isInstalled = app.installKey
                  ...
                  <div className="flex flex-1 flex-col justify-center">
                    ...
                    <div className="mt-1 flex items-center gap-3 text-xs text-white/50">
                      ...
                      {app.routes && app.routes.length > 0 && (
                        <span className="flex items-center gap-1">
                          📍 {app.routes.length}{" "}
                          {app.routes.length === 1 ? "route" : "routes"}
                        </span>
                      )}
                    </div>
                  </div>
