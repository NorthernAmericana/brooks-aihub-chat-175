...
const schema = z.object({ placeId: z.string().uuid(), note: z.string().max(500).optional(), mediaAssetId: z.string().uuid().optional() });
...
export async function POST(request: Request) {
  ...
  const visit = await db.execute<{ id: string }>(sql`
  ...
  return NextResponse.json({ visitId: visit[0]?.id ?? null, points: POINTS });
}
